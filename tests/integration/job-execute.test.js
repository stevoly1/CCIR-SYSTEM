...
describe('running one outbox entry', () => {
  ...
  it('records a retryable failure and leaves the entry to run again', async () => {
    behaviour.run.mockRejectedValue(JobError.of('PROVIDER_DOWN'));
    const entry = await add();
    await expect(executeEntry(entry._id, { runKey: 0, attempt: 1, maxAttempts: 3 })).rejects.toMatchObject({ code: 'PROVIDER_DOWN', final: false });
    expect(await OutboxEntry.findById(entry._id)).toMatchObject({ state: 'PENDING', attempts: 1, lastErrorCode: 'PROVIDER_DOWN' });
  ...
  it('skips done, failed, dismissed, missing and stale entries without running them', async () => {
    ...
    for (const entry of [done, failed, dismissed, stale]) {
